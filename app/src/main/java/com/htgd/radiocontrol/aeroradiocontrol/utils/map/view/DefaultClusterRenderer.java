...
package com.htgd.radiocontrol.aeroradiocontrol.utils.map.view;

import static com.htgd.radiocontrol.aeroradiocontrol.utils.map.algo.NonHierarchicalDistanceBasedAlgorithm.MAX_DISTANCE_AT_ZOOM;
...
import com.htgd.radiocontrol.aeroradiocontrol.utils.map.projection.Point;
import com.htgd.radiocontrol.aeroradiocontrol.utils.map.projection.SphericalMercatorProjection;
import com.htgd.radiocontrol.aeroradiocontrol.utils.map.ui.IconGenerator;
import com.htgd.radiocontrol.aeroradiocontrol.utils.map.ui.SquareTextView;
...
public class DefaultClusterRenderer<T extends ClusterItem> implements
    ...
    private LayerDrawable makeClusterBackground() {
        ...
        outline.getPaint().setColor(0xffffff); // Transparent white.  0x80ffffff
    ...
    public void onClustersChanged(Set<? extends Cluster<T>> clusters) {
        Log.e("wzq","zhixngxiayigetask  onClustersChanged");
    ...
    private class MarkerModifier extends Handler implements MessageQueue.IdleHandler {
        ...
        public void add(boolean priority, CreateMarkerTask c) {
            Log.e("wzq","zhixngxiayigetask  add");
            lock.lock();
                ...
                for (T item : cluster.getItems()) {
                    ...
                    if (marker == null) {
                        ...
                        if (animateFrom != null) {
                            ...
                            Log.e( "zhixngxiayigetask",item.getBitmapDescriptor().getBitmap().getByteCount()+"经纬度"+animateFrom.longitude+animateFrom.latitude);
                            markerOptions.icon(item.getBitmapDescriptor());
                        } else {

                            markerOptions.position(item.getPosition());
                            Log.e( "zhixngxiayigetasks",item.getBitmapDescriptor().getBitmap().getByteCount()+"经纬度" +item.getPosition().longitude+"纬度"+item.getPosition().latitude);
                            markerOptions.icon(item.getBitmapDescriptor());
                        }
