...
| **D-2026-06-01-16** | 2026-06-01 | **NEXT-2 鉴权 split-brain 闭环（kill-app 路径）** | 日常使用：登录后杀 app 重启，业务请求带空 URL → 后端 500 / "serveraddress must not be null"。补记 ledger（此前被各处引用但未正式入表）| AuthStore v2.1→**v2.2**（L1 明文 + L2 加密 EncryptedSharedPreferences 两层 + tokenExpiry + rememberMe + 4-path logout matrix + L2 四元组原子不变量）; NEW **StartupAuthDecider**（V4Activity.onCreate setContent 前同步 atomic check + ServerConfig.setBaseUrl rehydrate）; A2 backup_rules 排除 auth_secure.xml。Critic 5-cycle emulator PASS_W_MINOR HIGH。3-commit（dd3c0ad/ff55ed2/7b0d4f0）| **CLOSED**（code+test；真机 belt-and-suspenders 由 CTO 收口）|
| **D-2026-06-02-17** | 2026-06-02 | **NEXT-3 三线：真机 fix 轮 + 寻呼/点播 REST 调查 + Phase C 视觉修** | CTO 真机暴露 NEXT-2"已闭环"仍 live bug + 操作能用优先级 + 视觉不匹配 | ①**真机 fix**：★ `ServerToken.serverToken` 静态未 rehydrate = static-field-rehydration-trap **第二实例**（decider 加 `setAuthToken`）+ rememberMe 默认 false→**true** + ApiException typed 两层拦截（AuthInterceptor + V3CallbackAdapter HTML→NON_JSON）+ v3 活路入口 NO_SESSION 短路（绝不发空 token 请求，belt+suspenders）+ 作息全方案 SchemeSwitchButton（Handoff:929）。②**寻呼/点播 REST 调查**：寻呼/对讲音频面卡 native MP3 编码 → 确认挂 **R-001 arm64 慢线**；点播有 REST candidate `/task/addtemptask`（字段全匹配、服务器 200，但 state=0xFFF3 语义未知）→ **方案 A：不投机实现，待 CTO 真机点一次拿成功 state 码再一次到位**。③**视觉**：16 偏差查出，本轮修 7（**1 BLOCKER 终端网格 2→3 列** + 5 MAJOR Splash/MChip/ink2/瓦片青/字体 + kicker uppercase）；裁定项 #9 Login折叠/#10 密码长按/#15 渐变70vs80 CTO 待定。**critic 方法论补丁**：Leg1 必须核实际数值非仅 token 引用。Critic 双轮（NEXT-3 5-leg + 视觉 Opus 逐项 spec值→代码值 MATCH）PASS_W_MINOR HIGH | **EFFECTIVE**（fix CLOSED；点播待真机 state 码；视觉真机眼验 APK md5 745f935e）|
