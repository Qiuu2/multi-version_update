...
窗口本身是**无边框透明**的 —— 交接文档里的面板自带 12px 圆角、阴影和右上角的
关闭按钮，本来就是照无边框设计的，所以不再套一层系统标题栏。拖顶栏空白处可以移动窗口。

设置菜单（齿轮）里有「**桌面模式**」开关，打开后：

- 窗口**压到所有窗口之下**，不会再挡住你正在做的事
- 从**任务栏和 Alt+Tab 里隐去**，像桌面小组件一样待在桌面上

开关状态跟其他设置一起存盘，下次启动自动恢复。

这一层由 Tauri 的 `set_always_on_bottom` + `set_skip_taskbar` 实现（见
`src-tauri/src/lib.rs` 的 `set_desktop_mode`）。**它不是壁纸层**：按 Win+D
「显示桌面」时它会跟着被收起。要做到真正嵌进壁纸、待在桌面图标后面、且不受
「显示桌面」影响，得用 Windows 的 WorkerW 父窗口那套做法（Wallpaper Engine
的路子）—— 代价是窗口会变成桌面的子窗口，鼠标与键盘输入的路由会变得不可靠，
对这种需要勾选、拖拽、打字的面板不划算，所以没有采用。

浏览器里跑的时候这个开关不显示（没有窗口层级的概念）。
