...
窗口是**无边框透明**的 —— 交接文档里的面板自带 12px 圆角、阴影和右上角按钮，
本来就是照无边框设计的，所以不再套系统标题栏。顶栏（左侧一组、中间图例、整条栏的空白处）
都是拖拽区，按住可以拖动窗口。

齿轮菜单里有三项窗口相关设置：

**桌面模式** —— 窗口压到所有窗口之下，不再挡住正在做的事。

**面板大小** —— `−` / `+` 在 60% 到 120% 之间调档（60/70/80/90/100/110/120）。
整块面板等比缩放，窗口尺寸同步变成 `982 × 缩放` by `622 × 缩放`。
先调到顺眼的大小，再开桌面模式吸附上去。

**吸附到** —— 左上 / 右上 / 左下 / 右下一键归位，按显示器的**可用区**（`work_area`）计算，
不会被任务栏压住，四边各留 16px。手动拖动的位置也会记住，下次启动回到原处。

缩放的实现是给面板整体套 `transform: scale()`，而不是把布局改成流式的 ——
这样交接文档里那些定死的数值（单元格内边距、浮卡 248px、批量条 580px、各处夹取边界）
全部原样成立，只是整体放大缩小。代价是锚点换算要除掉缩放系数，
见 `CalendarPanel.tsx` 里的 `anchorOf` / `toPanel`。

### 托盘

应用**不在任务栏占位**（`skipTaskbar`），改从**通知区域的托盘图标**进出：

- 左键单击托盘图标：显示 / 隐藏
- 右键：显示 / 隐藏、吸附到右下角、退出
- 面板右上角的 `×` 是**收进托盘**，不是退出 —— 真正退出走托盘菜单的「退出」

### 一个限制

桌面模式是「压在最底层的窗口」，**不是壁纸层**：按 Win+D「显示桌面」时它会跟着被收起。
要做到连「显示桌面」都影响不了，得用 Windows 的 WorkerW 父窗口那套（Wallpaper Engine
的路子）—— 代价是窗口变成桌面的子窗口后，鼠标与键盘输入的路由会变得不可靠，
对这种要勾选、拖拽、打字的面板不划算，所以没有采用。
